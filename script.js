...
function calculateTypeDashboardTotals(items) {
    ...
    typeDashboardContainer.innerHTML = '';
    const tKey = headerMapping['type'];
    if (!tKey) return;
    ...
    sortedTypes.forEach(type => {
        ...
        const iconFilename = typeIconMap[type] || "other.png";
        
        const card = document.createElement('div');
        card.className = 'dash-card card-type';
        card.innerHTML = `
            <div style="margin-bottom: 10px;">
                <img src="${iconFilename}" alt="${type}" style="width: 40px; height: 40px; object-fit: contain;">
            </div>
